...
export const createSignature = async (req: Request, res: Response) => {
  ...
  if (existingSubscription) {
    return res
      .status(400)
      .json({ message: 'O usuário já possui uma assinatura ativa.' });
  }
  ...
  const newSubscription = await SignatureModel.create({
    ...
    end_date: new Date(
      new Date().setFullYear(
        new Date().getFullYear() + (plan === 'anual' ? 1 : 0.083)
      )
    ), // 1 ano para plano anual ou 1 mês para plano mensal
  });
...
export const getUserSubscriptions = async (req: Request, res: Response) => {
  ...
  if (subscriptions.length === 0) {
    return res
      .status(404)
      .json({ message: 'Nenhuma assinatura encontrada para este usuário.' });
  }
...
export const checkSubscriptionStatus = async (req: Request, res: Response) => {
  ...
  if (!subscription) {
    return res
      .status(404)
      .json({ message: 'Usuário não possui assinatura ativa.' });
  }
